...
| 1c | ENTREGADA 2026-08-21: semgrep primario (regla generada en runtime desde `$PROHIBIDOS`, una sola lista para dos motores), fallback que conserva el bloqueo, AMBER-4 y auto-escalada de CI | matriz completa §6 ✅ (10 tests, 5 rojos contra la versión anterior; mutante de la fusión con el grep visto en rojo) · aliases/whitespace/estrella ✅ · **FP <10% contra corpus KMP ajeno ✅ CERRADO 2026-08-22** (owner autorizó el clon): `compose-multiplatform` de JetBrains. La lista original dio **31 violaciones, las 31 falsas** (corrida del script, que por un `head -20` preexistente alcanzó 20 de los 57 `commonMain`; el reviewer rehízo la medición a escala completa: 1474 imports `androidx` con la lista vieja, 0 con la nueva, y el `head -20` se retiró en el mismo cambio con su test) — `androidx.` no es Android-only, Compose Multiplatform reusa ese namespace para iOS/desktop/web. Corregida a los subpaquetes solo-Android: **0 y 0** | implementador + owner (corpus) |
...
| 2b | ENTREGADA 2026-08-22: el paso 4 del bucle del mapa stagea por paths y separa los tres comandos; el mapa además se reconcilió (decía «En curso: nada» con este PRD activo) | ningún doc recomienda `-A` sin precondición ✅ — barrido completo: el mapa era el único infractor vivo; detector en `test_execution_map.sh` con precondición concreta (`git status` en ±1 línea) + 2 guards, verificado rojo contra el mapa anterior | implementador |
...
| 2026-08-24 | **DECISIÓN DEL OWNER sobre el freeze de §3: sigue VIGENTE y se hace cumplir.** Motivo: la métrica del ledger muestra que desde el 19 de agosto se abren 2–3× más hallazgos de los que se cierran, mientras los commits/día caían de 13–17 a 2–3 — el proyecto pasó de construir a auditar. Todo detector nuevo desde entonces había entrado "por la excepción declarada", así que la excepción se había comido la regla. A partir de aquí: **sin gates nuevos**, y el trabajo es la fase 3 (medir keep/tune/retire). PRD 0006 queda On hold; las vías 8 y 9 del bypass quedan como findings documentados | owner |
| 2026-08-22 | Gate de cierre de 1c CERRADO con corpus ajeno, y cambió el resultado: prohibir `androidx.` entero daba 100% de falsos positivos sobre el stack KMP más común. Corregido a los subpaquetes solo-Android. Lección nueva: un corpus escrito por quien construye el detector confirma sus errores en vez de exponerlos | agente del template |
| 2026-08-22 | **Nota de orden:** la ola 2 se entregó 2b antes que 2a, invirtiendo el orden de la tabla §5b. Se deja escrito, no normalizado — mismo trato que la inversión de 0c (AMBER-1). Motivo: 2b era pequeña y desbloqueaba el mapa, que se inyecta con autoridad en cada arranque y estaba mintiendo | agente del template |
| 2026-08-22 | Fase 2b entregada: staging por paths en el mapa + reconciliación de su estado (seis fases entregadas y el doc seguía diciendo «En curso: nada», que se inyecta con autoridad en cada arranque). Cierra `f-wf08-git-add-A-canonico`. Lección nueva: un patrón de detector con acentos da verde a mano y rojo en el runner, porque la suite corre en locale C | agente del template |
| 2026-08-21 | Fases 1b y 1c entregadas. 1b: `project_kind` declarado gobierna el scope (reviewer GREEN). 1c: semgrep kotlin primario con la tabla de fallback de §6 completa; la regla se genera en runtime en vez de vivir en `tools/semgrep/rules/kotlin.yaml` — desviación deliberada de §5, porque un YAML estático no ve las ampliaciones de `source-sets.conf` y dejaría dos listas divergiendo. Queda pendiente el FP contra corpus ajeno | agente del template |
