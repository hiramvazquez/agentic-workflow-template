...
- **Regla:** en el harness, **prefiere una sola ruta a la ruta óptima por plataforma**. Si la bifurcación es inevitable, cada rama necesita un test que **la fuerce en cualquier plataforma** —con un stub de la herramienta en el `PATH`— o la rama no probada se declara como cobertura ausente. Esto **no lo cubre** la respuesta de la lección del 2026-08-09 («que la suite corra en las dos plataformas»): esa depende de que ese CI exista y se ejecute, y hoy en este repo el Anillo 3 no ejecuta (`f-787568d2`). Un stub en el `PATH` no depende de nada. Y antes de aceptar «es flaky», **comprueba si alguna vez estuvo verde**: `git show <commit>^:<archivo> | grep -c` cierra la pregunta en dos comandos.
- **Detector:** `tools/tests/test_ring3.sh::test_un_timeout_del_sistema_no_relaja_el_limite` — pone en el `PATH` un `timeout` que imita a GNU sin `-k` (TERM al vencer, sin escalar) y exige que el límite se respete igual, así que se pone **rojo en macOS** si alguien vuelve a preferir un `timeout` del sistema sin escalada. Verificado con mutación real: con el mutante vivo el test viejo sigue ✅ y solo muere este. **Cobertura parcial declarada:** vigila esta bifurcación, no la clase — `tools/harness-report.sh:40` y `tools/secret-baseline.sh:66` siguen vivas y sin detector (`f-1caf51c6`).
...
- **Causa raíz:** la lista describía "qué NO es producto" con las categorías de un proyecto de
  app, y se aplicaba también donde esas categorías **son** el producto. Es `f-harness-no-autogate`
